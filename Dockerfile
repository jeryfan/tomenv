FROM ubuntu
RUN apt update && apt install -y wget curl openssh-server vim git xz-utils && mkdir /var/run/sshd && echo 'root:123' | chpasswd && sed -i 's/#PermitRootLogin prohibit-password/PermitRootLogin yes/' /etc/ssh/sshd_config
