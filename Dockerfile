...
# node
RUN mkdir -p ~/node && cd ~/node && wget https://nodejs.org/dist/v20.10.0/node-v20.10.0-linux-x64.tar.xz && tar -xvf node-* && mv node-*/* . && rm -rf node-* && echo NODE_PATH=/root/node/bin >> ~/.bashrc && echo 'PATH=$PATH:$NODE_PATH' >> ~/.bashrc
